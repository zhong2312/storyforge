...
- **27.1-c** 🟡 MVP 已可用（2026-07-11）：右侧 Agent Dock、按功能来源/自定义分组的本机会话历史、逐阶段推理摘要/工具详情/阶段输出/耗时、确认卡片、批准后写入与面板刷新、HTTP/SSE MCP 连接；跨设备会话同步与“编辑方案后重提案”仍待补
- **27.1-d** 🟡 第一批入口已接入（2026-07-11）：`dispatchAgentIntent` 将灵感反推、角色新建/补全、大纲卷章生成、正文生成/续写/扩写/润色/去 AI 味/按审校报告修改连到右侧 Agent Dock，并携带项目/世界/章节/实体/字段/选区作用域；自由对话可通过 `chapterIndex` 将“第 N 章”解析为规范章序和真实记录 ID 后继续读取/提案，宿主锁定作用域不可覆盖；其余 AI 入口继续按模块迁移
