...
- **27.1-b** 🟡 首个可用切片完成（2026-07-11）：AI SDK `ToolLoopAgent` + OpenAI-compatible provider + 多步工具循环、结构化事件、取消与审批恢复；已兼容 DeepSeek 仅接受 `tool_choice=auto` 的工具调用差异，并按阶段收窄可用工具；完全不支持 tool calling 的提供商降级仍待补
- **27.1-c** 🟡 MVP 已可用（2026-07-11）：右侧 Agent Dock、按功能来源/自定义分组的本机会话历史、按事件真实顺序穿插的阶段描述/推理摘要/工具详情/耗时、候选内容预览、采纳最终版本、输入调整要求后作废旧方案并重新生成、放弃、写入后面板刷新、HTTP/SSE MCP 连接；跨设备会话同步与通用字段级 diff 编辑仍待补
- **27.1-d** 🟡 第一批入口已接入（2026-07-11）：`dispatchAgentIntent` 将灵感反推、角色新建/补全、大纲卷章生成、正文生成/续写/扩写/润色/去 AI 味/按审校报告修改连到右侧 Agent Dock，并携带项目/世界/章节/实体/字段/选区作用域；正文入口附带结构化完成契约，未读全声明上下文、提案目标/章节错误、正文为空或过短时不能误报完成，章节字数从候选正文确定性派生；自由对话可通过 `chapterIndex` 将“第 N 章”解析为规范章序和真实记录 ID 后继续读取/提案，宿主锁定作用域不可覆盖；其余 AI 入口继续按模块迁移
- **统一项目存储后端** 🟡 Web/PWA 已完成（2026-07-12）：Workspace 的项目 store、AI 上下文、Agent 工具、导入导出、检索、事实账本及生命周期统一经 `ProjectStoragePort` 活动后端读写；数据管理可将 Dexie 项目逐表迁移到本地文件夹并校验数量与 SHA-256 指纹，成功后才切换后端；文件夹权限失效时要求重新授权，不静默回退旧 Dexie。Tauri 原生文件系统桥接待仓库建立 `src-tauri` 后接入同一端口。
