...
- `AiSdkAgentRuntimeAdapter` 已通过 `AgentRuntimePort` 接入 AI SDK `ToolLoopAgent`，支持多步工具循环、事件流、取消、提案暂停和审批恢复；写入型面板意图可声明结构化完成契约，运行时会保持工具循环并校验上下文覆盖、目标、记录、模式、必填字段和最低正文长度，未产生合法提案时不得发送成功事件。
- 受约束任务会按模型能力选择工具策略：支持强制工具的模型使用 `required/指定工具`；DeepSeek Chat Completions 使用 `auto`，并通过 `activeTools` 将阶段收窄为“读取上下文”或“提交提案”。
...
- Workspace 右侧 Agent Dock 按 `AgentEvent.sequence` 在对应阶段内依次显示阶段描述、推理摘要和工具调用，不再把聚合消息固定放到工具时间线之后；审批卡直接预览同一待提交计划中的候选正文。
- 审批交互支持“采纳最终版本 / 调整 / 放弃”：采纳后才调用 `change.commit` 并刷新面板；调整会先作废旧计划，再携带上一版候选和用户要求在同一会话生成新提案；放弃不修改项目。
- `storyforge.change.propose` 会从章节候选 `content` 确定性派生 `wordCount`，审批预览读取同一待提交计划输入，避免模型估算字数与实际正文不一致。
- 对话按原功能来源自动归入项目/设定/角色/大纲/正文/其他分组，支持自定义分组、重命名、移动和删除；最近会话与压缩后的有序阶段事件保存在当前设备，页面重载后可继续查看历史。
...
- 当前边界：会话历史为当前设备的 UI 缓存，未进入项目导出/跨设备同步；重载前未处理的审批会失效并要求重提案；其余 AI 入口仍需逐模块迁移；不支持 tool calling 的 provider 降级、Tauri stdio MCP、通用字段级 diff 编辑、多 Agent 编排和后台 Agent 尚未实施。
...
- 写入结果先预览；用户可采纳、放弃或输入调整要求重新生成，旧提案必须失效
- 验证：用户能"聊着把世界观建好"，面板实时刷新

**Phase 27.1-d 扩展对话覆盖面**
- 逐步接入：灵感对话反推、角色/大纲/正文的对话生成
- 写入工具全面接入确认机制
- 正文等必须产出可采纳结果的动作使用运行时完成契约，禁止在只读若干设定或输出“下一步”后假完成
