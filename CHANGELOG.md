...
## Unreleased — 2026-07-01 · 社区反馈修复批次（场景崩溃 / 主线 / 启动 / 主题 / 伏笔）
...
- **修复：Windows 本地启动出现「127.0.0.1 重定向次数过多」**。本地打开时自动清理历史遗留的 service worker 缓存（只清缓存，绝不动你的作品和 API 配置）；`启动.bat` 增加端口占用检测，被占用时提示先关掉旧窗口 / 旧 `StoryForge.exe`。
- **改进：Windows 小白启动引导**。README 新增「一键启动路径」——普通用户下载 Portable 包双击 `StoryForge.exe`、源码用户用 `启动.bat`，并附「重定向报错」自救步骤（强调不会删数据）。
...
## Unreleased — 2026-06-30 · 角色设计全面升级：23 维度体系 + AI 补全 + 反向哺喂
