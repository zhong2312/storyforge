...
export const FIELD_REGISTRY: FieldSpec[] = [
  ...
  longtext('characters', 'background', ['背景', '背景故事', 'backgroundStory']),
  longtext('characters', 'motivation', ['动机', '动机/欲望']),
  longtext('characters', 'abilities', ['能力', '能力/金手指']),
  longtext('characters', 'relationships', ['关系', '人物关系']),
  longtext('characters', 'arc', ['角色弧光', '成长线', '角色弧光/成长线']),
  // 扩展角色维度（CHARACTER_DIMENSIONS 描述符的写权威；AI 输出经 adopt() 写回）
  longtext('characters', 'identity', ['身份', '职业', '势力', '势力归属', '身份/职业/势力']),
  text('characters', 'profile', ['年龄性别', '年龄·性别·种族', '基础信息', '年龄', '性别', '种族']),
  longtext('characters', 'values', ['价值观', '信念', '价值观/信念']),
  longtext('characters', 'strengths', ['优点', '长处', '优点/长处']),
  longtext('characters', 'weaknesses', ['缺点', '弱点', '性格弱点', '缺点/性格弱点']),
  longtext('characters', 'fears', ['恐惧', '软肋', '逆鳞', '恐惧/软肋']),
  longtext('characters', 'goals', ['目标', '目标(短/长期)', '短期目标', '长期目标']),
  longtext('characters', 'innerConflict', ['核心矛盾', '核心矛盾/内心冲突', '内心冲突']),
  longtext('characters', 'keyEvents', ['关键经历', '关键经历/转折', '转折事件', '重要经历']),
  text('characters', 'powerLevel', ['实力定位', '境界', '等级', '战力', '实力定位/境界']),
  longtext('characters', 'speechStyle', ['语言风格', '口头禅', '说话方式', '语言风格/口头禅']),
  longtext('characters', 'habits', ['习惯', '小动作', '癖好', '习惯/小动作/癖好']),
  text('characters', 'signatureItem', ['标志性物品', '标志性物品/符号', '形象符号', '标志物']),
  text('characters', 'location', ['常驻地点']),
  text('characters', 'firstAppearance', ['首次出场']),
  longtext('characters', 'storyRole', ['作用', '角色作用', '在故事中的作用']),
  longtext('characters', 'ending', ['结局', '结局走向', '最终结局']),
