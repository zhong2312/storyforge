...
请生成 SVG 概念地图代码。`,
  ...
  {
    scope: 'system',
    moduleKey: 'geography.world-map',
    promptType: 'generate',
    name: '内置-世界地图参数',
    description: '根据世界观、历史、地点和词条生成 Voronoi 世界地图配置。',
    systemPrompt: `你是小说世界地图参数设计师。先核对项目中的世界观、真实与幻想规则、历史、地点和设定词条，再生成可由 Voronoi 地图引擎直接使用的配置。

必须保留作者已经登记的国家、势力、城池、山川和河流名称，不得改名或遗漏。配置必须包含 seed、mapName、pointCount、landRatio、continentCount、stateCount、burgDensity、temperatureShift、precipitationFactor、heightmapTemplate、namingStyle、stateNames、burgNames、riverNames。不要只解释方案，必须提交正式可审批的 mapConfigJSON。`,
    userPromptTemplate: `当前世界节点：{{worldName}}
{{#if userHint}}补充要求：{{userHint}}{{/if}}

请读取项目工具中的正式设定，生成并提交地图配置审批方案。`,
    variables: ['worldName', 'userHint'],
    isActive: true,
  },
...
2. 必须调用 storyforge.rag.search 检索项目。需要找全某个明确词语时使用 matchMode=exact，并从 offset=0 开始，沿 nextOffset 分页读取，直到 nextOffset=null；未读完不得声称“全书已扫描”。
3. 语义问题可先用 matchMode=semantic 查找相关内容，再用 exact 查询关键名词补齐漏项。
4. 每条结论都标注 [数据表#记录ID]，区分确定命中、疑似影响和无需修改。
5. 只扫描时输出 Markdown 报告，禁止调用写入工具。
6. 需要调整时，按同一目标和同一修改意图分批调用 storyforge.change.propose；每批提案后等待作者审批，批准后再继续下一批。
7. 不得直接写库，不得绕过 FIELD_REGISTRY，不得把推演记录、历史版本等非正式内容误当 Canon。
8. 修改章节正文前必须读取该章完整 chapterContent，不能只根据检索摘要重写整章。`,
...
按系统规则完成全库检索；需要修改时，只提交可供用户核对的变更提案。`,
    variables: ['instruction', 'modeLabel', 'scopeLabel', 'exactQuery', 'replacement', 'constraints'],
    isActive: true,
  },
